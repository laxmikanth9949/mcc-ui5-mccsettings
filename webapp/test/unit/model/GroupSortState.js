sap.ui.define([
	"mcc/workbench/admin/settings/model/GroupSortState",
	...
	QUnit.test("Should always return a sorter when sorting", function (assert) {
		...
		assert.strictEqual(this.oGroupSortState.sort("RoleName").length, 1, "The sorting by RoleName returned a sorter");
	});
	...
	QUnit.test("Should set the grouping to None if the user sorts by RoleName and there was a grouping before", function (assert) {
		// Arrange
		this.oModel.setProperty("/groupBy", "");

		this.oGroupSortState.sort("RoleName");
