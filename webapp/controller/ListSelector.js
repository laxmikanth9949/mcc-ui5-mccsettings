sap.ui.define([
	...
	return BaseObject.extend("mcc.workbench.admin.settings.model.ListSelector", {
		 ...
		 * @class
		 * @public
		 * @alias mcc.workbench.admin.settings.model.ListSelector
		 */
		...
		selectAListItem: function (sBindingPath) {
			...
			this.oWhenListLoadingIsDone.then(
				function () {
					...
					if (oSelectedItem) {
						if (oSelectedItem.getBindingContext() && oSelectedItem.getBindingContext().getPath() === sBindingPath) {
							return;
						} else if (oSelectedItem.getBindingContext("AdminApp") &&
							oSelectedItem.getBindingContext("AdminApp").getPath() === sBindingPath) {
							return;
						}
					}
					...
					oList.getItems().some(function (oItem) {
						if (oItem.getBindingContext() && oItem.getBindingContext().getPath() === sBindingPath) {
							...
							return true;
						} else if (oItem.getBindingContext("AdminApp") && oItem.getBindingContext("AdminApp").getPath() === sBindingPath) {
							oList.setSelectedItem(oItem);
							return true;
						}
					});
		 ...
		 * @return {mcc.workbench.admin.settings.model.ListSelector} the list selector object for method chaining
		 * @public
		 */
		...
		findListItem: function (sPath) {
			...
			for (var i = 0; i < aItems.length; i++) {
				...
				if (!(oItem instanceof GroupHeaderListItem)) {
					var oCtx = oItem.getBindingContext();
					if (sPath.includes("Phases")) oCtx = oItem.getBindingContext("AdminApp");
