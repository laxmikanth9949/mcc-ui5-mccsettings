sap.ui.define([
	...
	return BaseController.extend("mcc.workbench.admin.settings.controller.Workbench.Roadmap.RoadmapPhases", {
		...
		onInit: function () {
			...
			this._oListSelector = this.getOwnerComponent().oListSelectorWorkbenchRoadmap;
			...
			this.getRouter().getRoute("RoadmapPhases").attachPatternMatched(this._onMasterMatched, this);
			...
			this._oListSelector.oWhenListLoadingIsDone.then(
				function (mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("RoadmapTasks", {
							ID: encodeURIComponent(mParams.list.getAggregation("items")[1].getBindingContext("AdminApp").getProperty("ID"))
						}, true);
					}
				}.bind(this),
				function (mParams) {
		...
		_showDetail: function (oItem) {
			...
			this.getRouter().navTo("RoadmapTasks", {
				ID: encodeURIComponent(oItem.getBindingContext("AdminApp").getProperty("ID"))
			}, bReplace);
		},
		...
		_findItem: function () {
			...
			if (itemToSelect) {
				...
				if (!oItem) { //item is not viewable in the tree. not in the current tree page.
					...
					if (oItem) {
						sPath = oItem.getBindingContext("AdminApp").getPath();
					} else {
						this.getRouter().getTargets().display("detailNoObjectsAvailable");
						return;
					}
