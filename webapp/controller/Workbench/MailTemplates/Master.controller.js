...
sap.ui.define([
	...
	return BaseController.extend("mcc.workbench.admin.settings.controller.Workbench.MailTemplates.Master", {
		...
		onInit: function () {
			...
			this._oListSelector = this.getOwnerComponent().oListSelectorWorkbenchMailTemplates;
			...
			this.getRouter().getRoute("masterWorkbenchMailTemplates").attachPatternMatched(this._onMasterMatched, this);
		...
		onSearch: function (oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("TemplateName", FilterOperator.Contains, sQuery)];
			} else {
			...
			this._oListSelector.oWhenListLoadingIsDone.then(
				function (mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("objectWorkbenchMailTemplates", {
							TemplateID: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("TemplateID"))
						}, true);
					}
				}.bind(this),
				function (mParams) {
		...
		_showDetail: function (oItem) {
			...
			this.getRouter().navTo("objectWorkbenchMailTemplates", {
				TemplateID: encodeURIComponent(oItem.getBindingContext().getProperty("TemplateID"))
			}, bReplace);
		},
